      ...
      <section id="works" className="py-24 bg-[#161616]">
        <div className="max-w-6xl mx-auto px-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
            {WORKS.map((w) => (
              <div key={w.label} className="aspect-square bg-[#222] relative overflow-hidden group cursor-pointer">
                <img src={HERO_IMG} alt={w.label} className="w-full h-full object-cover opacity-60 group-hover:opacity-80 group-hover:scale-105 transition-all duration-500" />
                <div className="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent" />
                <div className={`absolute top-3 left-3 ${w.color} px-2 py-0.5`}>
                  <span className="font-display text-xs font-bold uppercase text-black">{w.label}</span>
                </div>
                <div className="absolute bottom-4 left-4 right-4">
                  <div className="font-display text-sm font-bold uppercase">{w.label}</div>
                </div>
              </div>
            ))}
          </div>
          <p className="text-center text-gray-500 font-body text-sm">Фотографии реальных объектов — добавьте их в раздел «Работы»</p>
        </div>
      </section>
